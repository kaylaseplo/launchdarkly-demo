...
function trackConversion() {
    ...
    client.flush();
    console.log(`\u{1F4CA} Conversion tracked: checkout_completed \u{2014} ${context.name} (${context.plan} plan)`);
}
window.trackConversion = trackConversion;
function renderCheckout(enabled, context) {
    const content = document.getElementById('feature-content');
    content.innerHTML = enabled ? premiumCheckout(context) : standardCheckout(context);
}
// ─────────────────────────────────────────────────────────────
...
function premiumCheckout(context) {
    ...
    <div style="display: flex; flex-direction: column; align-items: center; gap: 6px; flex: 1;">
      <div style="width: 32px; height: 32px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 14px; font-weight: 700;
        ${active ? 'background: rgba(255,255,255,0.95); color: #667eea;' : 'background: rgba(255,255,255,0.25); color: white;'}">${n}</div>
    ...
    <div style="max-width: 520px; margin: 20px auto; font-family: -apple-system, 'Segoe UI', Roboto, sans-serif; border-radius: 16px; overflow: hidden; box-shadow: 0 12px 40px rgba(102,126,234,0.35); background: white;">
        ...
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 22px;">
          <div>
            <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 4px;">
              <span style="font-size: 18px;">\u{1F680}</span>
          ...
          <span style="background: rgba(255,255,255,0.2); color: white; font-size: 11px; font-weight: 600; padding: 4px 10px; border-radius: 20px; letter-spacing: 0.5px;">\u{1F512} SECURE</span>
        </div>
      ...
      <div style="padding: 28px;">
        ...
        <div style="display: flex; align-items: center; gap: 12px; background: #f4f5fb; border-radius: 10px; padding: 12px 16px; margin-bottom: 22px;">
          ...
          <div>
            ...
            <div style="font-size: 12px; color: #7a7f95;">${context.accountType} account \xb7 ${context.email}</div>
          </div>
        </div>
        ...
        <div style="display: flex; gap: 10px; margin: 8px 0 18px;">
          <div style="flex: 1; border: 2px solid #667eea; border-radius: 10px; padding: 12px 14px; background: #f4f5fb; cursor: pointer;">
            <div style="font-weight: 600; color: #1f2437; font-size: 14px;">\u{1F4B3} Visa \u{2022}\u{2022}\u{2022}\u{2022} 4242</div>
            <div style="font-size: 12px; color: #7a7f95;">Expires 08/28 \xb7 Default</div>
          </div>
          <div style="flex: 1; border: 2px solid transparent; border-radius: 10px; padding: 12px 14px; background: #1a73e8; cursor: pointer; box-shadow: 0 2px 8px rgba(26,115,232,0.35);">
            <div style="font-weight: 600; color: white; font-size: 14px;">\u{1F535} Google Pay</div>
            <div style="font-size: 12px; color: rgba(255,255,255,0.85);">Fast, secure checkout</div>
          </div>
          <div style="flex: 1; border: 1px dashed #c4c8d8; border-radius: 10px; padding: 12px 14px; color: #667eea; font-size: 14px; font-weight: 600; display: flex; align-items: center; justify-content: center; cursor: pointer;">\u{FF0B} Add new card</div>
        </div>
      ...
      <div style="display: flex; justify-content: center; gap: 22px; padding: 14px 28px; background: #f4f5fb; border-top: 1px solid #eceef4; font-size: 12px; color: #7a7f95;">
        <span>\u{1F512} 256-bit encryption</span>
        <span>\u{26A1} One-click reorder</span>
        <span>\u{21A9}\u{FE0E} Free returns</span>
      </div>
    </div>`;
}
// ─────────────────────────────────────────────────────────────
...
function standardCheckout(context) {
    ...
    <div style="max-width: 460px; margin: 20px auto; font-family: Arial, Helvetica, sans-serif; border: 1px solid #ddd; border-radius: 6px; background: #fff;">
      <div style="padding: 16px 22px; border-bottom: 1px solid #e5e5e5; background: #f7f7f7; border-radius: 6px 6px 0 0;">
        <div style="font-size: 13px; font-weight: bold; color: #555; margin-bottom: 2px;">\u{1F680} Northwind Cloud</div>
        <h2 style="margin: 0; font-size: 18px; color: #333;">Secure Checkout</h2>
        <div style="font-size: 13px; color: #777; margin-top: 4px;">
          ${context.name} \xb7 ${context.accountType} account
        </div>
      </div>
      ...
      <div style="padding: 22px;">
        ...
        <div style="text-align: center; font-size: 12px; color: #999; margin-top: 10px;">\u{1F512} Secure checkout</div>
      </div>
    </div>`;
}
